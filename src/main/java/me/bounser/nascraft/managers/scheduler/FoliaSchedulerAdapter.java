...
public class FoliaSchedulerAdapter implements SchedulerAdapter {
    ...
    public int scheduleAsyncDelayed(Runnable task, long delayTicks) {
        try {
            ...
            if (usesConsumer) {
                Consumer<Object> consumer = (scheduledTask) -> task.run();
                Object taskObj = runDelayedAsync.invoke(asyncScheduler, plugin, consumer, delayMs, TimeUnit.MILLISECONDS);
                return taskObj.hashCode();
            } else {
                Object taskObj = runDelayedAsync.invoke(asyncScheduler, plugin, task, delayMs, TimeUnit.MILLISECONDS);
                return taskObj.hashCode();
            }
        } catch (Exception e) {
            plugin.getLogger().severe("Failed to schedule async delayed task: " + e.getMessage());
            return -1;
        }
    }
    ...
    public int scheduleGlobalDelayed(Runnable task, long delayTicks) {
        try {
            if (usesConsumer && !runDelayedGlobal.getName().equals("execute")) {
                Consumer<Object> consumer = (scheduledTask) -> task.run();
                Object taskObj = runDelayedGlobal.invoke(globalRegionScheduler, plugin, consumer, delayTicks);
                return taskObj.hashCode();
            } else {
                Object taskObj = runDelayedGlobal.invoke(globalRegionScheduler, plugin, task, delayTicks);
                return taskObj.hashCode();
            }
        } catch (Exception e) {
            plugin.getLogger().severe("Failed to schedule global delayed task: " + e.getMessage());
            return -1;
        }
    }
